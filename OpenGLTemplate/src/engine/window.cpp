...
namespace OGLWindow {
	...
	void addWindowCallbacks() {
		if (nativeWindow != nullptr) {
			...
			glfwSetKeyCallback(nativeWindow, OGLInput::keyCallback);					// Window key callback
			glfwSetCursorPosCallback(nativeWindow, OGLInput::mousePosCallback);			// Window mouse position callback
			glfwSetMouseButtonCallback(nativeWindow, OGLInput::mouseButtonCallback);	// Window mouse button callback
		}
	}
